...
class Base_datos:
    ...
    def añadir_bd(ind, obj):
        if (ind == 1):      # Añadir nueva Empresa
            archivo = open("Archivos/Empresas.txt", "a")
            archivo.write(str(obj.codigo) + "|" +
                          obj.nombre + "|" + obj.tipo + "|\n")
            archivo.close()
        elif (ind == 2):    # Añadir nuevo Producto
            archivo = open("Archivos/Productos.txt", "a")
            archivo.write(str(obj.cantidad) + "|" + obj.nombre + "|" + str(
                obj.fabricante.codigo) + "|" + obj.categoria + "|" + str(obj.precio) + "|\n")
            archivo.close()
        elif (ind == 3):    # Añadir nuevo Servicio
            archivo = open("Archivos/Servicios.txt", "a")
            archivo.write(str(obj.cupo) + "|" + obj.nombre + "|" + str(
                obj.proveedor.codigo) + "|" + obj.categoria + "|" + str(obj.precio) + "|\n")
            archivo.close()

    #-------------------------------- |Eliminar de la Base de Datos| --------------------------------#

    def eliminar_db(ind, obj):
        ...
        temp = open("Archivos/temp_administrativo.txt",
                    "w")  # Archivo temporal
        ...
        for linea in archivo:
            ...
            if (ind == 1):  # Si es una empresa
                if (datos[1] != obj.nombre):
                    temp.write(linea)
            else:
                if (ind == 2):  # Si es un producto
                    if (not((datos[1] == obj.nombre) & (datos[2] == obj.fabricante.codigo))):
                        temp.write(linea)
                else:  # Si es un servicio
                    if ((not(datos[1] != obj.nombre) & (datos[2] == obj.proveedor.codigo))):
                        temp.write(linea)
    ...
    def modificar_db(ind, obj):
        ...
        for linea in archivo:
            ...
            if (datos[1] != obj.nombre):
                ...
                if (ind == 1):
                    temp.write(str(obj.codigo) + "|" +
                               obj.nombre + "|" + obj.tipo + "|\n")
                elif (ind == 2):
                    temp.write(str(obj.cantidad) + "|" + obj.nombre + "|" + str(
                        obj.fabricante.codigo) + "|" + obj.categoria + "|" + str(obj.precio) + "|\n")
                elif (ind == 3):
                    temp.write(str(obj.cupo) + "|" + obj.nombre + "|" + str(
                        obj.proveedor.codigo) + "|" + obj.categoria + "|" + str(obj.precio) + "|\n")
        archivo.close()
        temp.close()
